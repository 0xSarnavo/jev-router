...
export const JevRouter = async ({ directory }) => {
  const notes = new Map();
  const roles = new Map();
  ...
  return {
    "chat.message": async (input, output) => {
      ...
      const res = run("prompt", { session_id: input.sessionID, prompt, cwd: directory, harness: "opencode",
    ...
    event: async ({ event }) => {
      ...
      if (event?.type === "session.idle" && replies.has(p.sessionID)) {
        run("stop", { session_id: p.sessionID, cwd: directory, harness: "opencode",
